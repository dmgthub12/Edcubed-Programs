                      ...
                      <article className={`program-row ${isTeacher ? "teacher-row" : ""}`} key={program.id}>
                        ...
                        <div className="program-row-copy">
                          <div className="tags compact-tags">
                            {isTeacher ? <><span className="green-tag">{roster.length} applied</span><span>{roster.filter((student) => normalizedStatus(student) === "under_review").length} under review</span><span>{roster.filter((student) => normalizedStatus(student) === "approved").length} approved</span></> : <><span className="green-tag">{applicationStatus(program.id)}</span><span>{program.schedule}</span><span>{program.time}</span><span>{seatText(program)}</span></>}
